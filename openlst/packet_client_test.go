...
func TestNewClientPacketToBytes_SmallFrame(t *testing.T) {
	...
	want := []byte{0x0a, 0xff, 0x03, 0x01, 0x00, 0xfd, 0x38, 0x0a, 0x0b, 0x0c, 0x0d}
	got := p.ToBytes()
	if !reflect.DeepEqual(want, got) {
...
func TestClientPacketFromBytes_SmallFrame(t *testing.T) {
	val := []byte{0x09, 0xff, 0x03, 0x04, 0x00, 0xfd, 0x38, 0x01, 0x02, 0x03}
...
func TestClientPacketFromBytes_EmptyFrame(t *testing.T) {
	val := []byte{0x06, 0xff, 0x03, 0x04, 0x00, 0xfd, 0x38}
